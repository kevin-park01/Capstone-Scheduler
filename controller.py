...
speakers = parser.parseSpeakers('speaker.csv')
sessions = parser.parseSession('session.csv')
rooms = parser.parseRooms('rooms.csv')
days = parser.parseDays('Date.csv')
start_times, end_times = parser.parseTime('time.csv')
...
def create_schedule():
    ...
    parser.generatedCSV(sessions_sched)
