...
speakers = parse.parseSpeakers('import/speaker.csv')
sessions = parse.parseSession('import/session.csv')
rooms = parse.parseRooms('import/rooms.csv')
days = parse.parseDays('import/Date.csv')
start_times, end_times = parse.parseTime('import/time.csv')
...
def create_schedule():
    ...
    filter_formats = day_schedule.get_room_formats()

    print(filter_equipment)

    for room, num_available in day_schedule.get_filtered_room_availability(filter_days, filter_times, filter_equipment, filter_capacity, filter_formats, sessions):
